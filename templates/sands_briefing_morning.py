...
for i in range(len(timed_events)):
    for j in range(i + 1, len(timed_events)):
        a = timed_events[i]
        b = timed_events[j]
        a_s = int(a['start'][:2]) * 60 + int(a['start'][3:])
        a_e = int(a['end'][:2]) * 60 + int(a['end'][3:])
        b_s = int(b['start'][:2]) * 60 + int(b['start'][3:])
        b_e = int(b['end'][:2]) * 60 + int(b['end'][3:])
...
def calc_free_hours(events, work_start="09:00", work_end="18:00"):
    ...
    for ev in events:
        if not ev['is_timed']:
            continue
        s = int(ev['start'][:2]) * 60 + int(ev['start'][3:])
        e = int(ev['end'][:2]) * 60 + int(ev['end'][3:])
