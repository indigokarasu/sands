...
calendars_to_query = ['google-workspace-user', '<family-calendar-id>@group.calendar.google.com']
...
When direct Calendar API returns 404 for a calendar ID that appears in MCP/Composio listings (e.g., TheTopaz `contact@example.com`, Family `<family-calendar-id>@group.calendar.google.com`), use Composio as an alternative query path:
