...
When direct Calendar API returns 404 for a calendar ID that appears in MCP/Composio listings (e.g., TheTopaz `<third-party-email>`, Family `<family-calendar-id>@group.calendar.google.com`), use Composio as an alternative query path:
