...
|---|---|---|---|
| Personal | `google-workspace-user` | Read (direct API + MCP + Composio) | The working personal calendar |
| TheTopaz | `contact@example.com` | Read (direct API + Composio) | Works via direct API as of 2026-06-17 |
| Family | `<family-calendar-id>@group.calendar.google.com` | Read (direct API + Composio) | Works via direct API as of 2026-06-17; earlier 404 was likely transient |
...
| Work | `owner.operator@<employer>.com` | FreeBusyReader only | Read-only (free/busy); cannot read event details |
...
3. Pass the calendar ID as shown in Composio's calendar list (e.g., `contact@example.com`, `<family-calendar-id>@group.calendar.google.com`)

This pattern worked for TheTopaz and Family calendars which returned 404 on direct API but returned events via Composio.

**Key difference**: Composio's calendar list shows `<family-calendar-id>@group.calendar.google.com` (no `.v`), while the earlier known-good table showed `@group.v.calendar.google.com`. Use the ID exactly as returned by Composio's list_calendars.